class Precinct extends ui.PrecinctUI {
    constructor(gameuser) {
        super();
        var box = this.getChildByName("user");
        console.log("box" + box);
        let name = box.getChildByName("name");
        let id = box.getChildByName("id");
        let image = box.getChildByName("touxiang");
    ...
    //建立游戏链接
    SocketRoom() {
        var ws = new WebSocket("ws://47.92.121.171:17788/JiuDianBan01");
        ws.onopen = function () {
            // Web Socket 已连接上，使用 send() 方法发送数据
            //user    值为 name+pas+belong
            console.log("fasong");
            ws.send('0068{"code":"1_1_001","msg":{"which":"N","user":"eeeeee123456aabbccdd"}}');
        };
